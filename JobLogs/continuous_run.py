...
def create_files():
    hops_path = os.environ['SLURM_COST_DIR'] + 'hops.txt'
    debug_path = os.environ['SLURM_COST_DIR'] + 'debug.txt'
    
    if os.path.exists(hops_path) or os.path.exists(debug_path):
        sys.exit("Hops and Debug files exist. Please move content.")
    
    hops = open(hops_path,'w')
